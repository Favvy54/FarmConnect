...
export default function WhyChoose() {
  return (
    <section id="benefits" className="px-3 py-16 md:px-6">
      ...
      <p className=" text-body2 md:text-body1 text-body-text mt-3 max-w-2xl">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
        {FEATURES.map(({ icon: Icon, title, body }) => (
          <div
            key={title}
            className="rounded-2xl border-2 border-green-normal px-6 py-8 flex flex-col items-center text-center">
            <AdvancedImage cldImg={Icon} className="w-8 h-8 mb-4" alt={title} />
            <h3 className="text-body1 font-bold text-green-normal mb-2">{title}</h3>
            <p className="text-body2 text-green-normal">{body}</p>
          </div>
        ))}
      </div>
    </section>
